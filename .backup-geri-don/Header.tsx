          ...
          <nav className="hidden lg:flex items-center gap-0">
            {navItems.map((item) => (
              <Link
                key={item.key}
                href={item.href}
                className={`px-3 py-2 text-[15px] font-bold tracking-wide uppercase whitespace-nowrap transition-colors ${
                  pathname === item.href
                    ? "text-gold"
            ...
            <button
              ...
              className="px-3 py-1 text-[11px] font-bold tracking-wider text-charcoal/40 hover:text-charcoal transition-colors uppercase"
            >
              {locale === "tr" ? "EN" : "TR"}
            </button>
          </nav>
